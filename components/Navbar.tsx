    ...
    <header className="sticky top-0 z-50 backdrop-blur-md bg-white/90 dark:bg-slate-950/90 border-b border-slate-200/80 dark:border-slate-800/80 transition-colors shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 gap-2">
          ...
          <Link href="/" className="flex items-center gap-2 sm:gap-3 group select-none min-w-0 flex-shrink">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-purple-600 flex items-center justify-center text-white shadow-md shadow-blue-500/20 group-hover:scale-105 group-hover:shadow-blue-500/35 transition-all duration-200 flex-shrink-0">
              <div className="relative flex items-center justify-center">
                <Calendar className="w-4 h-4 sm:w-5 sm:h-5" />
                <Clock className="w-3 h-3 sm:w-3.5 sm:h-3.5 absolute -bottom-1 -right-1 text-amber-300 drop-shadow-sm" />
              </div>
            </div>
            <div className="flex flex-col min-w-0">
              <span className="text-sm sm:text-lg md:text-xl font-black tracking-tight text-slate-900 dark:text-white flex items-center gap-0.5 sm:gap-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate">
                {SITE_CONFIG.name}
                <span className="text-blue-600 dark:text-blue-400">.</span>
              </span>
              <span className="text-[9px] sm:text-[10px] uppercase tracking-wider font-semibold text-slate-400 -mt-0.5 truncate">
                {t('appTagline')}
              </span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            ...
            <button
              ...
              className="p-1.5 sm:p-2 rounded-xl text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors touch-manipulation"
      ...
      <div className="border-t border-slate-200/70 dark:border-slate-800/70 bg-slate-50/80 dark:bg-slate-900/75 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
          <nav 
            ...
            className="flex items-center gap-1.5 sm:gap-2 py-1.5 sm:py-2 overflow-x-auto no-scrollbar scroll-smooth touch-pan-x px-1"
          >
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`group flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all duration-200 ease-out border shadow-sm min-h-[36px] sm:min-h-[40px] touch-manipulation ${
                    isActive
