...
import DateAddSubtractCalculator from '@/components/calculators/DateAddSubtractCalculator';
...
const meta = CALCULATORS_CATALOG['date-calculator'];

export const metadata: Metadata = {
  title: meta.title,
  description: meta.description,
  alternates: {
    canonical: `${SITE_CONFIG.url}/date-calculator`,
  },
  openGraph: {
    title: meta.title,
    description: meta.description,
    url: `${SITE_CONFIG.url}/date-calculator`,
  },
};

export default function DateCalculatorPage() {
  const webAppSchema = generateCalculatorSchema('date-calculator');
  const faqSchema = generateFaqSchema(meta.faqs);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Home', url: SITE_CONFIG.url },
    { name: meta.h1, url: `${SITE_CONFIG.url}/date-calculator` },
  ]);
  ...
  return (
    <>
      ...
      <div className="space-y-8">
        ...
        <AdPlaceholder slotType="horizontal" adSlotId="date-add-sub-banner" />

        <SeoContentSection
          toolName={meta.h1}
          faqs={meta.faqs}
          guideTitle="How Adding and Subtracting Dates Works"
        />
      </div>
    </>
  );
