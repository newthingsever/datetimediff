...
const meta = CALCULATORS_CATALOG['days-between-dates'];

export const metadata: Metadata = {
  title: meta.title,
  description: meta.description,
  alternates: {
    canonical: `${SITE_CONFIG.url}/days-between-dates`,
  },
  openGraph: {
    title: meta.title,
    description: meta.description,
    url: `${SITE_CONFIG.url}/days-between-dates`,
  },
};

export default function DaysBetweenPage() {
  const webAppSchema = generateCalculatorSchema('days-between-dates');
  const faqSchema = generateFaqSchema(meta.faqs);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Home', url: SITE_CONFIG.url },
    { name: meta.h1, url: `${SITE_CONFIG.url}/days-between-dates` },
  ]);
  ...
  return (
    <>
      ...
      <div className="space-y-8">
        ...
        <AdPlaceholder slotType="horizontal" adSlotId="days-between-banner" />

        <SeoContentSection
          toolName={meta.h1}
          faqs={meta.faqs}
          guideTitle="How to Count Days Between Dates Accurately"
        />
      </div>
    </>
  );
